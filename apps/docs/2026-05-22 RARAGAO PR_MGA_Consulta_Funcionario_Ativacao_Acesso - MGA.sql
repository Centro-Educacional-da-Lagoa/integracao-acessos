...
/*
  Consulta de funcionários elegíveis para concessão de acesso TOTVS.

  Regras principais:
  - situação funcional elegível para ativação:
    - A = ativo
    - Z = admissão futura já efetivada e com data de admissão vencida
  - login correto do funcionário continua sendo o CPF
  ...
  Exemplo:
  EXEC [dbo].[PR_MGA_Consulta_Funcionario_Ativacao_Acesso] '2026', null, null, null
  EXEC [dbo].[PR_MGA_Consulta_Funcionario_Ativacao_Acesso] '2026', 'MARIA', null, null
  EXEC [dbo].[PR_MGA_Consulta_Funcionario_Ativacao_Acesso] '2026', null, 1, '000123'
*/

CREATE OR ALTER PROCEDURE [dbo].[PR_MGA_Consulta_Funcionario_Ativacao_Acesso]
		...
		select	 func.codcoligada													as CD_Coligada
				...
				, row_number() over
					(
						...
						order by case
									when func.codsituacao = 'A' then 0
									when func.codsituacao = 'Z' then 1
									else 9
								 end
								, func.dataadmissao desc
								, func.chapa desc
					)																as NR_Linha
